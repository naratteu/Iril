...
namespace Iril
{
    ...
    class InnerGlobalInitializersCompiler : Emitter
    {
        ...
        public void Compile()
        {
            ...
            // Special case: initialize @stdout, @stderr, @stdin with real FILE* handles
            // by calling StdLib functions that allocate console streams
            var stdioSymbols = new[] {
                ("@stdout", "@_iril_get_stdout_ptr"),
                ("@stderr", "@_iril_get_stderr_ptr"),
                ("@stdin",  "@_iril_get_stdin_ptr")
            };
            foreach (var (globalSym, stdlibFunc) in stdioSymbols)
            {
                // Use the interned symbol (starts with '@', so it's a GlobalSymbol)
                var sym = (GlobalSymbol)Symbol.Intern(globalSym);
                if (info.GlobalFields.TryGetValue(sym, out var gfInfo))
                {
                    try
                    {
                        var initMethod = compilation.GetSystemMethod(Symbol.Intern(stdlibFunc));
                        if (info.DataField == null) continue;
                        // Call the StdLib function → FILE* handle on stack
                        Emit(il.Create(OpCodes.Call, initMethod));  // stack: [file_handle]
                        // Save the file_handle to a temp var
                        var tmpV = new VariableDefinition(compilation.sysBytePtr);
                        body.Variables.Add(tmpV);
                        Emit(il.Create(OpCodes.Stloc, tmpV));  // save, stack: []
                        // Load the data pointer and store handle to the field
                        Emit(il.Create(OpCodes.Ldsfld, info.DataField));  // stack: [DataStruct*]
                        Emit(il.Create(OpCodes.Ldloc, tmpV));  // stack: [DataStruct*, file_handle]
                        Emit(il.Create(OpCodes.Stfld, gfInfo.Field));  // store, stack: []
                    }
                    catch { }
                }
            }
