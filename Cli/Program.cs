...
namespace Cli
{
    class Program
    {
        ...
        /// <summary>
        /// Preprocesses LLVM IR (clang-14 through current) into a form Iril's parser accepts.
        /// - Replaces 'fneg' (LLVM 10+) with equivalent 'fsub' form.
        /// - Strips dereferenceable_or_null attributes.
        /// - Blanks attribute-group bodies (attributes #N = { ... }). Iril discards these
        ///   hint attributes entirely, so emptying them insulates the parser from the
        ///   ever-evolving function-attribute syntax (memory(...), uwtable(sync), nofpclass(...), ...).
        /// </summary>
        static string PreprocessLLVMIR(string ll)
        {
            ...
            // Empty out attribute-group definition bodies (single line, no '}' inside).
            ll = System.Text.RegularExpressions.Regex.Replace(
                ll,
                @"^(attributes #\d+ = \{)[^}]*(\})",
                "$1 $2",
            ...
            // Drop LLVM 19+ debug-record statements (#dbg_declare/#dbg_value/#dbg_label),
            // the intrinsic-free successors to @llvm.dbg.*. Iril ignores debug intrinsics anyway.
            ll = System.Text.RegularExpressions.Regex.Replace(
                ll,
                @"^\s*#dbg_\w+\([^\n]*\)\s*$\n?",
                "",
