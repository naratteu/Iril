...
namespace Iril
{
    public class Compilation
    {
        ...
        public bool TryGetFunction(Module module, Symbol symbol, out DefinedFunction function)
        {
            ...
            if (moduleMethodDefs.TryGetValue(module.Symbol, out var mdefs) && mdefs.TryGetValue(symbol, out function))
            {
                function.ReferenceCount++;
                var deps = syscalls.GetDependencies(symbol);
                foreach (var d in deps) externalMethodDefs[d].ReferenceCount++;
                return true;
            }
            // 2. Check external definitions (StdLib + declarations)
            if (externalMethodDefs.TryGetValue(symbol, out function))
            {
                function.ReferenceCount++;
                var deps = syscalls.GetDependencies(symbol);
                foreach (var d in deps) externalMethodDefs[d].ReferenceCount++;
            ...
            // 3. Search all other modules (cross-module references when compiling multiple .c files)
            foreach (var kvp in moduleMethodDefs)
            {
                if (kvp.Value.TryGetValue(symbol, out function))
                {
                    function.ReferenceCount++;
                    return true;
                }
        ...
        class EntrypointEmitter : Emitter
        {
            ...
            public void Run()
            {
                ...
                var mainParams = maind.IRDefinition.Parameters;
                bool hasArgcArgv = mainParams.Length == 2
                    && mainParams[0].ParameterType is Types.IntegerType
                    && mainParams[1].ParameterType is Types.PointerType;

                if (hasArgcArgv)
                {
                    // Convert .NET string[] args to C-style argc/argv via StdLib.Args helper
                    bool emittedArgs = false;
                    try
                    {
                        var initMethod = compilation.GetSystemMethod(Symbol.Intern("@_iril_args_init"));
                        var getArgcMethod = compilation.GetSystemMethod(Symbol.Intern("@_iril_get_argc"));
                        var getArgvMethod = compilation.GetSystemMethod(Symbol.Intern("@_iril_get_argv"));
                        il.Append(il.Create(OpCodes.Ldarg_0));  // string[] args
                        il.Append(il.Create(OpCodes.Call, initMethod));
                        il.Append(il.Create(OpCodes.Call, getArgcMethod));  // argc (int32)
                        il.Append(il.Create(OpCodes.Call, getArgvMethod));  // argv (byte**)
                    ...
                    if (!emittedArgs)
                    {
                        foreach (var p in mainParams) EmitZeroValue(p.ParameterType);
                        il.Append(il.Create(OpCodes.Call, maind.ILDefinition));
                    }
                }
                else
